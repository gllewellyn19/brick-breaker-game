...
/**
 * Tests for bricks that increases the ball speed
 *
 * @author Grace Llewellyn
 */
class IncreaseBallSpeedBrickTest extends DukeApplicationTest {
  ...
  void brickFunctionTest() {
    double initialBallSpeed = currentLevel.getBall().getBallSpeed();
    ballShape.setCenterX(270);
    ballShape.setCenterY(150);
    currentLevel.getBall().setXDirection(-1);
    currentLevel.getBall().setYDirection(-1);
    paddleShape.setX(200);
    press(myScene, KeyCode.SPACE);
    myGame.step(.5);
    myGame.step(2);
    javafxRun(() -> myGame.step(.5));
    assertTrue(currentLevel.getBall().getBallSpeed()!=initialBallSpeed);
  }
